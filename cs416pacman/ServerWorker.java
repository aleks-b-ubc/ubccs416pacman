...
class ServerWorker implements Runnable {
	...
	ServerWorker(Socket client, Node serverNode, int ghostID) {
	...
	public void run() {
		...
		while (threadRun) {
			try {
				// will keep track of inactivities. After a second if not
				// reading anything
				// in we will test connection. If during that time ANYTHING
				// comes in
	...
	private void testConnection() {
		...
		while (!gotAns && (numFail < FAILLIMIT)) {
			...
			// one of the directions. Then connection is alive. If what we get
			// is not
			// the test number, then it is a direction number and we set the
			// direction.
